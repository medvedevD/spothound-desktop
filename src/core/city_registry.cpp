...
static const core::CityGeo kCities[] = {
    {
...
const core::CityGeo* core::CityRegistry::find(const std::string& cityName)
{
    const std::string norm = core::Rules::norm(cityName);
    if (norm.empty()) return nullptr;

    for (int i = 0; i < kCityCount; ++i) {
        const char* p = kCities[i].names;
        while (*p) {
            const char* comma = std::strchr(p, ',');
            std::string alias = comma ? std::string(p, comma) : std::string(p);
            if (norm.find(alias) != std::string::npos
                    || alias.find(norm) != std::string::npos) {
                return &kCities[i];
            }
            p = comma ? comma + 1 : p + alias.size();
        }
    }
    return nullptr;
}
